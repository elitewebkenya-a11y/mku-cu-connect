    ...
    <section className="py-8 bg-gradient-to-br from-muted/30 via-background to-muted/20">
      <div className="container mx-auto px-4">
        <div className="text-center mb-5">
          ...
          <h2 className="text-xl md:text-2xl font-serif font-bold text-foreground">Meet Our Leaders</h2>
        </div>
          ...
          {leaders.map((leader) => (
            <Card key={leader.id} className="group overflow-hidden border-0 shadow-sm hover:shadow-md transition-all duration-300 bg-card">
              ...
              <CardContent className="p-2 md:p-3 text-center">
                <h3 className="font-bold text-xs md:text-sm text-foreground line-clamp-1">{leader.name}</h3>
                <p className="text-[10px] md:text-xs text-muted-foreground line-clamp-1">{leader.position}</p>
              </CardContent>
            </Card>
          ))}
        </div>
